...
export default function SystemUpdates({ minToShow = 1 }) {
    ...
    return (
        <div className={global.landing}>
            <div className={global.subcentered}>System Updates</div>

            <div className={global.list}>
                {updates.map((u) => (
                    <div key={u.id} className={global.card}>
                        <div className={global.itemHeader}>
                            <div>{u.title}</div>
                        ...
                        {u.critical && <div className={global.critical}>⚠ Critical</div>}

                        <div className={global.description}>{u.body}</div>
                    </div>
                ))}
            </div>
        </div>
